            ...
            Ok(mut stream) => {
                ...
                let encrypted_response: Vec<u8> = match &decryption_method {
                    ...
                    DecryptionMethod::Ed25519(_) => {
                        // Ed25519 encryption placeholder
                        response.as_bytes().to_vec()
                    }
                    DecryptionMethod::Symmetric(_) => {
                        // Symmetric encryption placeholder
                        response.as_bytes().to_vec()
                    }
...
let authorized_ips: Option<Vec<IpAddr>> = match authorized_ips {
    Some(ips) => Some(ips.into_iter().map(|n| n.ip()).collect()),
    None => Some(vec!["127.0.0.1".parse().unwrap()]), // allow localhost for testing
};
