    ...
    for stream in listener.incoming() {
        match stream {
            Ok(mut stream) => {
                ...
                if verbose {
                    println!("Sending {} bytes response to {}", encrypted_response.len(), peer_addr);
                }
