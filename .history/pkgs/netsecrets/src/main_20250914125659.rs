...
fn start_server(
    ...
    if stream.read_to_end(&mut buffer).is_ok() {
        ...
        let client_pub_key = parts.next(); 
                ...
                // FIX: Don't try to encrypt responses for asymmetric encryption
                // since the client may not be able to decrypt them
        let encrypted_response = if let Some(client_pub_key_b64) = client_pub_key {
            match &decryption_method {
                DecryptionMethod::Rsa(_) => {
                    // Client provided RSA public key
                    let client_pub_key_pem = BASE64.decode(client_pub_key_b64).unwrap();
                    let client_pub_key = RsaPublicKey::from_pkcs1_pem(
                ...
                DecryptionMethod::Ed25519(_) => {
                    ...
                    let client_pub_key_bytes = BASE64.decode(client_pub_key_b64).unwrap();
                    let client_pub_key = X25519PublicKey::from(
                        <[u8; 32]>::try_from(client_pub_key_bytes.as_slice()).unwrap()
                    );
