...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::Receive {
            ...
            // If authorized_ips is Option<Vec<IpNetwork>>
            let authorized_ips: Option<Vec<IpAddr>> = authorized_ips.map(|v| {
                v.into_iter().map(|n| n.ip()).collect()
            });

            // Convert authorized_keys from Option<PathBuf> -> Option<Vec<String>>
