...
let request_bytes = match &decryption_method {
    DecryptionMethod::Symmetric(key) => decrypt_data(encrypted_request, key),
    DecryptionMethod::Rsa(_) | DecryptionMethod::Ed25519(_) | DecryptionMethod::None => {
        encrypted_request.to_vec()
    }
};
