...
let request_str = std::str::from_utf8(request).unwrap_or("").trim();
if verbose {
    println!("Received {} bytes from {}: {}", n, peer_addr, request_str);
}

// Split by whitespace: "<password> <key>"
let mut parts = request_str.split_whitespace();
let _recv_password = parts.next(); // optional, if you want to verify
let key_requested = parts.next().unwrap_or("");
