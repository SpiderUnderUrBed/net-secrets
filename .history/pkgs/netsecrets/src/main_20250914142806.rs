...
pub fn start_server(
    ...
    port: u16,
    password: Option<String>,
    map: Option<MyMapType>,
    verbose: bool,
    authorized_ips: Option<Vec<String>>,
        ...
        match stream {
            Ok(mut stream) => {
                ...
                if let Some(ref ips) = authorized_ips {
                    if !ips.contains(&peer_addr.ip().to_string()) {
                ...
                let encrypted_response: Vec<u8> = match &decryption_method {
                    ...
                    DecryptionMethod::Symmetric(_) => {
                        // fallback if you want to support sym key encryption too
                        response.as_bytes().to_vec()
                    }
                };
