...
Commands::Send {
    ...
    // FIX: Determine decryption method separately for responses
    let decryption_method = if insecure {
        ...
        // For asymmetric encryption when sending, we can't decrypt responses
        // unless we have the private key, so we'll use no encryption for responses
        if verbose {
