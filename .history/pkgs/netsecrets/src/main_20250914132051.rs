...
fn send_request(
    ...
    if stream.read_to_end(&mut response).is_ok() {
        ...
        // Try to decrypt response only if we have the proper decryption method
        let response_data = if matches!(decryption_method, DecryptionMethod::None) {
            // Response is plaintext
            if verbose {
                println!("Response is plaintext");
            }
            response
        } else {
        ...
        if verbose {
            println!("Decrypted response: {}", response_str);
        }
        ...
        eprintln!("Failed to read response from server");
    }
