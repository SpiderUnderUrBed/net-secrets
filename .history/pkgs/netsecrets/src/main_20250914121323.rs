...
fn load_ed25519_public_key(path: &Path) -> Ed25519PublicKey {
    ...
    Ed25519PublicKey::from_bytes(&bytes).expect("Failed to parse Ed25519 public key")
}

fn load_ed25519_private_key(path: &Path) -> Ed25519SecretKey {
    let b64 = fs::read_to_string(path).expect("Failed to read Ed25519 private key file");
    let bytes = BASE64.decode(b64.trim()).expect("Failed to decode base64");
    Ed25519SecretKey::from_bytes(&bytes).expect("Failed to parse Ed25519 private key")
}

fn derive_x25519_from_ed25519_public(ed25519_key: &Ed25519PublicKey) -> X25519PublicKey {
    let bytes = ed25519_key.to_bytes();
    X25519PublicKey::from(bytes)
}

fn derive_x25519_from_ed25519_secret(ed25519_key: &Ed25519SecretKey) -> StaticSecret {
    let bytes = ed25519_key.to_bytes();
    ...
    for stream in listener.incoming() {
        if let Ok(mut stream) = stream {
            ...
            if stream.read_to_end(&mut buffer).is_ok() {
                ...
                let encrypted_response = encrypt_data(response.as_bytes(), &encryption_method);
...
fn main() {
    ...
    match cli.command {
        ...
        Commands::Send {
            ...
            let decryption_method = match &encryption_method {
                EncryptionMethod::None => DecryptionMethod::None,
                EncryptionMethod::Symmetric(key) => DecryptionMethod::Symmetric(key.clone()),
                _ => {
                    panic!("For asymmetric encryption, you need to specify both public and private keys");
                }
            };
