...
let request_str = std::str::from_utf8(request_bytes).unwrap_or("").trim();
if verbose {
    println!("Received {} bytes from {}: {}", n, peer_addr, request_str);
}

// extract the last word as the secret key
let key_requested = request_str.split_whitespace().last().unwrap_or("");
