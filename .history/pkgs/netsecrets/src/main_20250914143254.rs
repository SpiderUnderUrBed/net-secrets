...
Commands::Receive {
    ...
    // Convert authorized_ips from Vec<IpNetwork> -> Vec<IpAddr>
    let authorized_ips: Option<Vec<IpAddr>> = authorized_ips.map(|ips| {
        ips.into_iter().filter_map(|n| Some(n.ip())).collect()
    });
