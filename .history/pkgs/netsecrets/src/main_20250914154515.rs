...
fn determine_encryption_method(_insecure: bool, encryption_key: Option<String>) -> EncryptionMethod {
    if let Some(key_str) = encryption_key {
        let key = derive_symmetric_key(key_str.as_bytes());
        EncryptionMethod::Symmetric(key)
    } else {
...
fn determine_decryption_method(_insecure: bool, encryption_key: Option<String>) -> DecryptionMethod {
    if let Some(key_str) = encryption_key {
        let key = derive_symmetric_key(key_str.as_bytes());
        DecryptionMethod::Symmetric(key)
    } else {
