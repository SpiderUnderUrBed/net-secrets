...
fn generate_keys(output_dir: Option<PathBuf>) {
    ...
    fs::write(dir.join("encryption_key.hex"), &key_hex)
...
fn derive_symmetric_key(input: &[u8]) -> Vec<u8> {
...
fn determine_encryption_method(
    ...
    let key = if let Some(key_str) = encryption_key {
        if key_str.len() == 64 {
            // If it's a 64-character hex string, decode it directly
            hex::decode(&key_str).expect("Invalid hex encryption key")
        } else {
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(mut stream) => {
                ...
                // Allow any IP if authorized_ips is None or empty
                ...
                let request_bytes = match decrypt_data(encrypted_request, &decryption_method) {
                    Ok(data) => data,
                    Err(e) => {
                        eprintln!("Failed to decrypt request from {}: {}", peer_addr, e);
                        continue;
                    }
                };
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match cli.command {
        ...
        Commands::Send {
            ...
            // For decryption, we need to use the same key that was used for encryption
        ...
        Commands::Receive {
            ...
            // Convert authorized_ips to Vec<IpAddr> and allow any IP if empty
