...
// decrypt the request first
// decrypt the request first
let request_bytes = match &decryption_method {
    DecryptionMethod::Symmetric(_) | DecryptionMethod::Rsa(_) | 
    DecryptionMethod::Ed25519(_) | DecryptionMethod::None => {
        // always call decrypt_data with the full method reference
        decrypt_data(encrypted_request, &decryption_method)?
    }
};
                ...
                let encrypted_response: Vec<u8> = match &decryption_method {
                    DecryptionMethod::Rsa(_) => {
                        if let Some(keys) = &authorized_keys {
                            if let Some(client_pub_key_pem) = keys.first() {
                                if let Ok(client_pub_key) =
                                    RsaPublicKey::from_pkcs1_pem(client_pub_key_pem)
                                {
                                    if verbose {
                                        println!("Encrypting with client's RSA key");
                                    }
                                    encrypt_data(
                                        response.as_bytes(),
                                        &EncryptionMethod::Rsa(client_pub_key),
                                    )
                                } else {
                    ...
                    DecryptionMethod::Ed25519(_) => {
                        if let Some(keys) = &authorized_keys {
                            if let Some(client_pub_key_b64) = keys.first() {
                                if let Ok(client_pub_key_bytes) = BASE64.decode(client_pub_key_b64) {
                                    if client_pub_key_bytes.len() == 32 {
                                        let mut arr = [0u8; 32];
                                        arr.copy_from_slice(&client_pub_key_bytes);
                                        let client_pub_key = X25519PublicKey::from(arr);
                                        if verbose {
                                            println!("Encrypting with client's X25519 key");
                                        }
                                        encrypt_data(
                                            response.as_bytes(),
                                            &EncryptionMethod::Ed25519(client_pub_key),
                                        )
                                    } else {
                                        response.as_bytes().to_vec()
                                    }
                    ...
                    DecryptionMethod::Symmetric(key) => {
                        if verbose {
                            println!("Encrypting with symmetric key");
                        }
                        encrypt_data(response.as_bytes(), &EncryptionMethod::Symmetric(key.clone()))
                    }
                    DecryptionMethod::None => {
                        if verbose {
                            println!("Sending plaintext");
                        }
                        response.as_bytes().to_vec()
                    }
                };
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match cli.command {
        Commands::GenerateKeys { key_type, output_dir } => {
            // generate_keys returns (), so no ?
        ...
        Commands::Send {
            ...
            // ❌ do NOT use ? on a function that returns ()
            send_request(
                server,
                port,
                password,
        ...
        Commands::Receive {
            ...
            let server = get_env_var_or_required_arg("NETSECRETS_SERVER", server, "server").to_string();
            let port = get_env_var_or_arg("NETSECRETS_PORT", port).unwrap_or(8080);
            ...
            // Convert secrets vec into HashMap
            ...
            // Convert Vec<IpNetwork> -> Option<Vec<IpAddr>>
            let authorized_ips: Option<Vec<IpAddr>> = Some(
                authorized_ips.into_iter().map(|n| n.ip()).collect()
            );

            // Convert Option<PathBuf> -> Option<Vec<String>>
            let authorized_keys: Option<Vec<String>> = authorized_keys.map(|path| {
