    ...
    if stream.read_to_end(&mut buffer).is_ok() {
        ...
        let client_pub_key = parts.next(); 
                ...
                let encrypted_response = match &decryption_method {
                    DecryptionMethod::Rsa(_) | DecryptionMethod::Ed25519(_) => {
                        // For asymmetric encryption, respond in plaintext
                        // since the client may not have the private key to decrypt
                        if verbose {
                            println!("Responding with plaintext for asymmetric encryption");
                        }
                        response.as_bytes().to_vec()
                    }
                    _ => {
                        // For symmetric encryption or no encryption, use the normal flow
