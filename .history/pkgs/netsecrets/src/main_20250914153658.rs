...
use aes_gcm::aead::{rand_core::OsRng, Aead, KeyInit, Nonce};
...
fn encrypt_data(data: &[u8], method: &EncryptionMethod) -> Vec<u8> {
    match method {
        ...
        EncryptionMethod::Symmetric(key) => {
            ...
            let nonce = aes_gcm::Nonce::<Aes256Gcm>::from_slice(&nonce_bytes);
...
fn decrypt_data(data: &[u8], method: &DecryptionMethod) -> Result<Vec<u8>, String> {
    match method {
        ...
        DecryptionMethod::Symmetric(key) => {
            ...
            let nonce = aes_gcm::Nonce::<Aes256Gcm>::from_slice(&data[0..12]);
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(mut stream) => {
                ...
                // Allow all IPs if none specified
