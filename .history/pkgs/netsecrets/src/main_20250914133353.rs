...
// Encrypt response with the same method used for decryption
let encrypted_response = match &decryption_method {
    DecryptionMethod::Symmetric(key) => {
        // Use the same symmetric key for response
        if verbose {
            println!("Encrypting response with symmetric key");
        }
        encrypt_data(response.as_bytes(), &EncryptionMethod::Symmetric(key.clone()))
    }
    DecryptionMethod::Rsa(private_key) => {
        // For RSA, we need to check if client provided a public key
        if let Some(client_pub_key_b64) = client_pub_key_b64 {
            // Try to use client's public key
            ...
            // No client public key, use plaintext
            response.as_bytes().to_vec()
        }
    }
    DecryptionMethod::Ed25519(static_secret) => {
