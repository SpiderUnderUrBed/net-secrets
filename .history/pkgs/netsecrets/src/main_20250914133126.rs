...
fn start_server(
    ...
    if verbose && !authorized_keys.is_empty() {
        println!("Loaded {} authorized keys", authorized_keys.len());
    }
    ...
    for stream in listener.incoming() {
        if let Ok(mut stream) = stream {
            ...
            if stream.read_to_end(&mut buffer).is_ok() {
                if verbose {
                    println!("Received {} bytes from client", buffer.len());
                }

let decrypted_request = match decrypt_data(&buffer, &decryption_method) {
    Ok(data) => data,
    Err(e) => {
        if verbose {
            eprintln!("Failed to decrypt request: {}", e);
        }
        return; // Or handle the error appropriately
                ...
                let client_pub_key_b64 = parts.next();
                let client_name = parts.next();
...
let client_authorized = if authorized_keys.is_empty() {
    // No authorized_keys file provided, allow all clients
    true
} else if let Some(name) = client_name {
    // Check against authorized_keys
    authorized_keys.contains_key(name)
} else {
    // Client didn't provide name but we have authorized_keys
    false
};
...
if verbose {
    println!(
        ...
        if client_authorized || authorized_keys.is_empty() {
            "granted"
        } else {
            "denied"
        }
                ...
                // Only encrypt response if client is authorized
                let encrypted_response = if client_authorized {
                    // Use client's public key from authorized_keys file
                    let client_pub_key = authorized_keys.get(client_name.unwrap()).unwrap();
                    if verbose {
                        println!("Encrypting response with authorized client's RSA public key");
                    }
                    encrypt_data(response.as_bytes(), &EncryptionMethod::Rsa(client_pub_key.clone()))
                } else if let Some(client_pub_key_b64) = client_pub_key_b64 {
                    // Client provided public key but not authorized - try to use it anyway
                    match &decryption_method {
DecryptionMethod::Rsa(_) => {
    // Client provided RSA public key
    if let Ok(client_pub_key_data) = BASE64.decode(client_pub_key_b64) {
        // Try to parse as PEM first
        let client_pub_key_result = if String::from_utf8_lossy(&client_pub_key_data).contains("BEGIN RSA PUBLIC KEY") {
            // It's PEM format
            RsaPublicKey::from_pkcs1_pem(&String::from_utf8(client_pub_key_data).unwrap())
        } else {
            // It's raw DER format
            RsaPublicKey::from_pkcs1_der(&client_pub_key_data)
        };
        
        if let Ok(client_pub_key) = client_pub_key_result {
            if verbose {
        ...
        response.as_bytes().to_vec()
                ...
                eprintln!("Failed to read from client: {}", peer_addr);
            }
        }
    }
}
    ...
    match stream.read_to_end(&mut response) {
        Ok(_) => {
            ...
            let response_data = if matches!(decryption_method, DecryptionMethod::None) {
                // Client expects plaintext, but check if response might be encrypted
                if response.len() == 256 && verbose {
                    // This looks like an RSA-encrypted response (256 bytes)
                ...
                // Client has decryption method, try to decrypt
                if verbose {
